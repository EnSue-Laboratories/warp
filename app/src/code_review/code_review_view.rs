...
use warpui::elements::{
    ...
    DispatchEventResult, DragBarSide, Element, Empty, EventHandler, Fill, Flex, Hoverable, List,
...
impl CodeReviewView {
    ...
    fn render_file_header(
        ...
        if let Some(checkbox) = self.render_file_include_checkbox(file, appearance, app) {
            left_section.add_child(Container::new(checkbox).with_margin_right(8.).finish());
        }
    ...
    fn render_file_include_checkbox(
        ...
        let file_path = file.file_diff.file_path.clone();
        Some(render_include_checkbox(
            file.include_checkbox_mouse_state.clone(),
            is_included,
            appearance,
            move |ctx| {
                ctx.dispatch_typed_action(CodeReviewAction::ToggleFileIncluded(file_path.clone()));
            },
        ))
    }
            ...
            CodeReviewAction::ToggleAllFilesIncluded => {
                ...
                if let Some(repo) = self.active_repo.as_mut() {
                    if included_paths.is_empty() {
                        ...
                        repo.file_excluded = loaded.file_states.keys().cloned().collect();
                    }
                }
                self.update_git_operations_ui(ctx);
                ctx.notify();
