...
pub struct CommitState {
    ...
    /// Remote repos leave this as `None`; the daemon protocol does not carry a
    /// selected path set.
    ...
    /// When `Some`, the user pressed Confirm with an empty editor and we owe
    /// them a commit using whatever AI generation produces.
    pending_confirm_after_autogen: Option<CommitChainMode>,
}
                ...
                match result {
                    Ok(mut entries) => {
                        if let Some(filter) = &explicit_filter {
                            entries.retain(|entry| filter.contains(&entry.path));
                        }
...
pub(super) fn is_ready_to_confirm(state: &CommitState, app: &AppContext) -> bool {
    if !has_committable_changes(state) {
        return false;
    }
    if commit_message(state, app).is_some() {
        return true;
    }
    // Empty message is OK if the user can press Confirm and have us generate
...
pub(super) fn maybe_start_commit_message_autogen(
    ...
    let include_unstaged = match me.mode_mut() {
        GitDialogMode::Commit(state) => {
            if state.ai_autogen_in_flight {
                return;
            }
            state.ai_autogen_in_flight = true;
            state.include_unstaged
        }
        _ => return,
    };
...
pub(super) fn start_confirm(me: &mut GitDialog, ctx: &mut ViewContext<GitDialog>) {
    ...
    if commit_message(state, ctx).is_none() {
        ...
        if !ai_in_flight {
            maybe_start_commit_message_autogen(me, ctx);
        }
        return;
    }
...
fn reload_file_changes(me: &mut GitDialog, ctx: &mut ViewContext<GitDialog>) {
    ...
    let (include_unstaged, explicit_filter) = match me.mode() {
        GitDialogMode::Commit(state) => (
            state.include_unstaged,
            state
                .explicit_files
