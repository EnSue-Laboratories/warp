...
fn user_facing_git_error(raw: &str) -> String {
    ...
    let summary = if lower.contains("no changes added to commit") {
...
impl GitDialog {
    pub fn new_for_commit(
        ...
        let local_repo_path = repo_location.to_local_path();
        // Selected-file commits depend on local index staging. Remote git ops go
        // through the daemon protocol, which does not carry a selected path set.
        let explicit_files = if local_repo_path.is_some() {
            explicit_files
        } else {
            None
        };
        ...
        commit::maybe_start_commit_message_autogen(&mut this, ctx);
    ...
    /// Reverses `set_loading`: restores the confirm label and re-enables the
    /// dialog's buttons so the user can interact (or retry).
